# Signal report: definition of done

Grade only the files in /mnt/session/outputs/. Each criterion passes or fails.

1. **Report exists and covers the list.** `signals.md` exists and says, for every account in the task's list, either the signal(s) found or "nothing new". `signals.json` parses and matches it.
2. **Every signal is real and fresh.** Each signal has a source URL and a date inside the window the task gives. No signal is undated.
